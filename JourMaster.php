...
<?php
	...
	mysqli_query($con,"create table if not exists JournalTb(Jour_id varchar(255), name varchar(255),type varchar(255), category varchar(255), publisher varchar(255), NoOfPages int,cost varchar(255),primary key(Jour_id))") or die(mysql_error());
	
		$rs=mysqli_query($con,"select count(*) from JournalTb ");
		$c=0; 
	...
	if(isset($_POST['sbinsert']))
	{
		...
		mysqli_query($con,"insert into JournalTb(Jour_id,name,type,category,publisher,NoOfPages,cost) values('$id','$name','$type','$category','$publisher',$page, '$cost')") or die(mysql_error());
